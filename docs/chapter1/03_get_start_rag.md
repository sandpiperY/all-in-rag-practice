...
# 加载环境变量，从 .env 文件读取配置（如 API 密钥）
load_dotenv()

# 配置全局设置：设置使用的语言模型为 DeepSeek，并传入 API 密钥
Settings.llm = DeepSeek(model="deepseek-chat", api_key=os.getenv("DEEPSEEK_API_KEY"))
# 配置嵌入模型：使用 HuggingFace 上的中文文本嵌入模型
Settings.embed_model = HuggingFaceEmbedding("BAAI/bge-small-zh-v1.5")

# 读取文档：从指定路径加载 Markdown 文件作为文档数据
documents = SimpleDirectoryReader(input_files=["../../data/C1/markdown/easy-rl-chapter1.md"]).load_data()

# 创建向量索引：将文档内容转换为向量表示并构建可检索的索引结构
index = VectorStoreIndex.from_documents(documents)

# 创建查询引擎：基于向量索引构建问答接口，用于执行自然语言查询
query_engine = index.as_query_engine()

# 打印查询引擎使用的提示词模板，了解其内部工作方式
